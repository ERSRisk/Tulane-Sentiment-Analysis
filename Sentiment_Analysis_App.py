                ...
                with st.expander('View Risk Labels'):
                    ...
                    with st.expander("Manually update risk labels:"):
                        ...
                        with st.form(f"manual_edit_form_{idx}"):
                            ...
                            with col2:
                                upd_acceleration_value = st.number_input('Acceleration',  min_value=0.0, max_value = 5.0, step = 1.0, value=article['Acceleration_value_Upd'] if pd.notna(article['Acceleration_value_Upd'],key =f"acceleration_input_{idx}")
                            with col3:
                                upd_source_accuracy =st.number_input('Source Accuracy',  min_value=0.0, max_value = 5.0, step = 1.0, value= article['Source_Accuracy_Upd'] if pd.notna(article['Source_Accuracy_Upd'],key =f"source_input_{idx}")
                            with col4:
                                upd_impact_score = st.number_input('Impact Score',  min_value=0.0, max_value = 5.0, step = 1.0, value=aarticle['Impact_Score_Upd'] if pd.notna(article['Impact_Score_Upd'],key =f"impact_input_{idx}")
                            with col5:
                                upd_location=st.number_input('Location',  min_value=0.0, max_value = 5.0, step = 1.0, value=article['Location_Upd'] if pd.notna(article['Location_Upd'],key =f"location_input_{idx}")
                            with col6:
                                upd_industry_risk = st.number_input('Industry Risk',  min_value=0.0, max_value = 5.0, step = 1.0, value=article['Industry_Risk_Upd'] if pd.notna(article['Industry_Risk_Upd'],key =f"industry_input_{idx}")
                            with col7:
                                upd_frequency_score = st.number_input('Frequency Score', min_value=0.0, max_value = 5.0, step = 1.0, value=article['Frequency_Score_Upd'] if pd.notna(article['Frequency_Score_Upd'],key =f"frequency_input_{idx}")

...
def get_cookies():
    driver = uc.Chrome(headless = False, version_main = 136)
    ...
    time.sleep(10)
    WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.ID, "username"))).send_keys(os.getenv("email"))
    ...
    for feed in feeds:
        ...
        async def process_entry(entry, source):
            try:
                ...
                entities = [ent.text for ent in doc.ents if ent in relevant_entities]
                text_to_check = (
                    entry.title,
